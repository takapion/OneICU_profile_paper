...
overall_diag_cat as (
  select
    ...
    count(*) as count,
    round(count(*) / (select count(distinct icu_stay_id) from `snapshots_one_icu_derived.extended_icu_diagnoses_20250628` where category is not null), 1) as proportion
  from recategorize
